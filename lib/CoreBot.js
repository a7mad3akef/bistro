    ...
    function Conversation(task, message) {
        ...
        this.addMessage = function(message, thread, typing) {
            ...
            if (typeof(typing) !== 'undefined' && typing) {
                ...
                typingLength = 120 / 60 * message.text.length;
                typingLength = typingLength > 50 ? 50 : typingLength;
            }
        ...
        this.beforeThread = function(thread, callback) {
            ...
            this.before_hooks[thread] = callback;
          };
        ...
        this.gotoThread = function(thread) {
            ...
            if (that.before_hooks && that.before_hooks[thread]) {
 
              this.before_hooks[thread](this, function() {
 
                 if (!that.hasThread(thread)) {
                     if (thread == 'default') {
                         that.threads[thread] = [];
                     } else {
            ...
            that.messages = that.threads[thread].slice();
        ...
        this.tick = function() {
            ...
            if (this.isActive()) {
                if (this.processing) {
                   console.log('.. I am waiting for the topic to change');
                    ...
                    if (this.messages.length) {
                        ...
                        if (typeof(this.messages[0].timestamp) == 'undefined' ||
                            ...
                            if (message.text || message.attachments || message.attachment) {
                                var withTyping = message.with_typing;
                                delete message.with_typing;
                                ...
                                var messageCallback = function(err, sent_message) {
                                    if (err) {
                                ...
                                if (withTyping) {
                                    this.task.bot.replyWithTyping(this.source_message, outbound, messageCallback);
                                    } else {
                                    this.task.bot.reply(this.source_message, outbound, messageCallback);
                                    }
                            }
                            if (message.action) {
