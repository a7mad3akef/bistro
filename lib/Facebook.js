...
var detectMobile = require(__dirname + '/detectMobile'); //unused
const path = require('path');
var ejs = require('ejs');
//var fs = require('fs'); // write/read file on server
var XMLHttpRequest = require("xmlhttprequest").XMLHttpRequest

// related to passport authentification. Check if user is looged in before sending to personal space
...
function Facebookbot(configuration) {
    ...
    facebook_botkit.createWebhookEndpoints = function(webserver, bot, cb) {
        ...
        // Facebook login dialog, specifies scope
        webserver.get('/auth/fb',  (req, res, next) => {
            req.session.senderId = req.query.senderId;
            passport.authenticate('facebook',{ scope: ['user_friends', 'email', 'user_tagged_places', 'read_custom_friendlists', 'user_photos', 'user_work_history', 'user_hometown', 'ads_read', 'ads_management']}, 
            {
                state: {
        ...
        webserver.get('/auth/fb/callback', 
                
                passport.authenticate('facebook', {
                    successRedirect : '/schedule',
                    failureRedirect : '/https://m.me/' + process.env.app_name 
                }));

        //  User's personal space (loan schedule)
        webserver.get('/schedule', isLoggedIn, function(req, res) {
            ...
            req.on("end", function() {
                console.log(req.session.senderId);
                var redirect = {
                  ['user'] : req.session.senderId,
                  ['channel'] : req.session.senderId
                };
                console.log('Redirecting to user ' + redirect.user);
                
                bot.startConversation(redirect, successAuth); 
                
                bot.startConversation(redirect, paymentDateReminder); 
            });
            req.addEventListener("loadend", console.log("CLOSING"))
            req.on("close", function() {console.log("HEY HEY I CLOSED"); bot.say(req, 'Thanks for logging in');  }); // should send to next thread (add a question)
            req.addEventListener("loadend", console.log("CLOSING"));
        });
    ...
    facebook_botkit.setupWebserver = function(port, cb) {
        ...
        facebook_botkit.webserver.use(cookieParser());
        facebook_botkit.webserver.use(session({ secret: 'keyboard cat', resave: true, saveUninitialized: true}));
        facebook_botkit.webserver.use(passport.initialize());
        facebook_botkit.webserver.use(passport.session());
        facebook_botkit.webserver.set('views', __dirname + '/views');
